% Бүлэг 3

\chapter{Системийн зохиомж ба хөгжүүлэлт} % Зарим нэг зөвлөмж
\label{Chapter3} % Энэ бүлэг рүү ишлэл хийх бол \ref{Chapter2} командыг ашигла 

\section{Системийн архитектур}
\paragraph{} 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/arhcdiagram.png}
  \caption{EguneAI лого}
  \label{fig:architecturediagram}
\end{figure}
\section{Зохиомжийн шатны класс диаграмм}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/class_diagram.png}
  \caption{EguneAI лого}
  \label{fig:classdiagram}
\end{figure}
\section{Өгөгдлийн ерөнхий схем }

\section{Системийн прототип}

% \clearpage
\section{Хөгжүүлсэн системийн интерфейс}
\textbf{Landing хуудас}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/ui/landing.png}
  \caption{Landing хуудас}
  \label{fig:landingpage}
\end{figure}

\textbf{Нэвтрэх хуудас}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/ui/login.png}
  \caption{Нэвтрэх хуудас}
  \label{fig:loginpage}
\end{figure}

\textbf{Бүртгүүлэх хуудас}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/ui/login.png}
  \caption{Бүртгүүлэх хуудас}
  \label{fig:signuppage}
\end{figure}

\textbf{Үндсэн хуудас}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\linewidth]{figures/chapter3/ui/login.png}
  \caption{Үндсэн хуудас}
  \label{fig:homepage}
\end{figure}

\section{Системд хийгдсэн тест}
%альфа бета тестүүд ямар хэмжээнд хийгдсэн талаар, хийгдсэн тестүүдийн талаар
\section{Системийн нэвтрүүлэлт}
%Системийг бодит хэрэглэгч дээр ямар хэмжээнд нэвтрүүлсэн талаар


\section{Бүлгийн дүгнэлт}
